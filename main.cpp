...
void PaintImage(Film* film)
{
    ...
    for (size_t lin = 0; lin < resY; lin++)
    {
        ...
        for (size_t col = 0; col < resX; col++)
        {
            ...
            Vector3D random_color = Vector3D(col / (double)resX, lin / (double)resY, 0.0); //Vector3D((double)rand() / RAND_MAX, (double)rand() / RAND_MAX, (double)rand() / RAND_MAX);            
...
int main()
{
    ...
    Shader* whittedintegrator = new WhittedIntegrator(bgColor);
    Shader* hemisphericaldirshader = new HemisphericalDirShader(intersectionColor, bgColor);
    Shader* areadirshader = new AreaDirShader(intersectionColor, bgColor);
    Shader* purepathtracing = new PurePathTracing(intersectionColor, bgColor);
    ...
    //PaintImage(film);  comentar qun no es fa la task one
    ...
    //raytrace(cam, hemisphericaldirshader, film, myScene.objectsList, myScene.LightSourceList);
    raytrace(cam, areadirshader, film, myScene.objectsList, myScene.LightSourceList);
    //raytrace(cam, purepathtracing, film, myScene.objectsList, myScene.LightSourceList);
