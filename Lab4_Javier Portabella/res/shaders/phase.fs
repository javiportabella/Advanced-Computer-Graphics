...
void main()
{
    ...
    uniform float u_g; // Anisotropy factor for the HG phase
    ...
    while (t < tFar) {
        ...
        if (int(u_density_type) == 0) {
            ...
            // In-scattered dispersion with HG phase function
            float cosTheta = dot(rayDir, lightDir); // Angle between rayDir and lightDir
            float phaseHG = (1.0 - u_g * u_g) / pow(1.0 + u_g * u_g - 2.0 * u_g * cosTheta, 1.5);
            phaseHG /= 4.0 * 3.14159265359; // Normalization
            ...
            vec3 scatteringContribution = mu_s * inScatteredLight * phaseHG;   //Multiply the scattered light contribution by the value of the HG phase function
